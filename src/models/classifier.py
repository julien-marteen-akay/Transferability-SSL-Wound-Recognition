...
class Classifier(L.LightningModule):
    ...
    def get_classification_layer(self):
        ...
        # accumulate all shapes to be reached from start to end of mlp (both inclusive)
        # e.g. [2048, 512, 10] means we need Linear layers (2048, 512) and (512, 10)
        # if layers=1 --> []; if layers=2 --> [512] e.g. for self.num_mlp_layers=512
        in_to_out = [self.num_mlp_neurons for _ in range(self.num_mlp_layers - 1)]
    ...
    def set_metrics(self, stage, task="multiclass"):
        ...
        setattr(self, f"{stage}_rocauc",
                AUROC(num_classes=self.num_classes, task=task, average="macro", thresholds=None))
