...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        df.loc[:, "class_index"] = label_enc.fit_transform(df.loc[:, target_col])
    ...
    def set_stratified_split_indices(self, label_column: str = "class_index", val_split: float = 0.1,
        ...
        assert val_split > .0 and test_split > 0., (
            "I do not allow the flexibility of discarding validation and test sets."
    ...
    def __getitem__(self, index):
        return (
            self.transform(self.df[self.path_col].iloc[index]),
            self.target_transform(self.df["class_index"].iloc[index])
        )
...
class DataModule(L.LightningDataModule):
    ...
    def create_dataset(self) -> Dataset:
        ...
        dataset = Dataset(
            ...
            transform=get_transforms(augmentations=augmentation, keep_aspect_ratio=self.keep_aspect_ratio),
    ...
    def prepare_data(self) -> None:
        ...
        self.num_classes = self.create_dataset().num_classes  # property
    ...
    def setup(self, stage=None) -> None:
        ...
        self.train_dataset = torch.utils.data.Subset(dataset, dataset.train_indices)
        self.val_dataset = torch.utils.data.Subset(dataset, dataset.val_indices)
        self.test_dataset = torch.utils.data.Subset(dataset, dataset.test_indices)

        # remove augmentation and select from the val and test datasets
        self.val_dataset.dataset.set_to_eval_mode(keep_aspect_ratio=self.keep_aspect_ratio_eval)
        self.test_dataset.dataset.set_to_eval_mode(keep_aspect_ratio=self.keep_aspect_ratio_eval)
